(() => {
  ...
  style.textContent = `
    ...
    #${ROOT_ID} .cp-minimize {
      margin-left:auto;width:28px;height:28px;border:0;border-radius:8px;
    ...
    #${FAB_ID} {
      ...
      width:44px;height:44px;border:1px solid #dbe3ee;border-radius:14px;
      background:#fff;color:#111827;box-shadow:0 8px 24px rgba(15,23,42,.14);
      font:800 16px/1 system-ui,-apple-system,sans-serif;cursor:pointer;
  ...
  function applyState() {
    ...
    if (fab) fab.style.display = minimized ? 'grid' : 'none';
  }
  ...
  function start() {
    ...
    if (localStorage.getItem(MINIMIZED_KEY) === 'true') makeFab();
    applyState();
