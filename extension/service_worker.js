...
chrome.runtime.onInstalled.addListener(() => {
  chrome.alarms.create('classpilot-sync', { periodInMinutes: 10 });
});
...
async function connectToClassPilot() {
  ...
  let tab = tabs[0];
  if (!tab?.id) {
    tab = await chrome.tabs.create({ url: CLASS_PILOT_URL, active: true });
  } else {
  ...
  return { ok: false, message: 'Open ClassPilot in this tab, make sure you are signed in, then click Connect again.' };
}

async function syncPayload(payload) {
  const { classPilotAccessToken } = await getAuth();
  if (!classPilotAccessToken) return { ok: false, message: 'Connect to ClassPilot once from the extension menu first.' };
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  (async () => {
    ...
    if (message?.type === 'SYNC_ACTIVE_SCHOOLOGY') {
      ...
      if (!tabs[0]?.id) { sendResponse({ ok: false, message: 'Open Schoology in a tab first.' }); return; }
      try {
        const result = await chrome.tabs.sendMessage(tabs[0].id, { type: 'SYNC_NOW' });
        sendResponse(result || { ok: true, message: 'Sync started.' });
      } catch (_) { sendResponse({ ok: false, message: 'Refresh the Schoology tab once, then try again.' }); }
      return;
    }
...
chrome.alarms.onAlarm.addListener(async alarm => {
  if (alarm.name !== 'classpilot-sync') return;
  const tabs = await chrome.tabs.query({ url: ['https://*.schoology.com/*', 'https://schoology.com/*'] });
  if (!tabs[0]?.id) return;
  try { await chrome.tabs.sendMessage(tabs[0].id, { type: 'SYNC_NOW' }); } catch (_) {}
});
