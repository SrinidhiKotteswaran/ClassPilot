(() => {
  ...
  root.innerHTML = `
    <div style="display:flex;align-items:center;gap:9px">
      <div style="width:30px;height:30px;border-radius:9px;background:#0f172a;color:#fff;display:grid;place-items:center;font-weight:800">C</div>
      <div><div style="font-weight:800">ClassPilot</div><div id="cp-sub" style="font-size:11px;color:#64748b">Schoology sync</div></div>
    </div>
    <div id="cp-status" style="margin-top:10px;color:#475569">Checking your Schoology data…</div>
    <button id="cp-sync" style="width:100%;margin-top:10px;padding:9px 12px;border:0;border-radius:10px;background:#0f172a;color:#fff;font-weight:700;cursor:pointer">Sync now</button>
  `;
  ...
  function parseCourses(doc, baseUrl) {
    ...
    doc.querySelectorAll('a[href]').forEach(a => {
      ...
      const id = courseId(href); if (!id) return;
  ...
  function parseAssignments(doc, course) {
    ...
    doc.querySelectorAll('a[href]').forEach(a => {
      const href = absolute(a.getAttribute('href'));
      const id = assignmentId(href); if (!id) return;
  ...
  async function sync() {
    ...
    try {
      ...
      sub.textContent = 'Needs connection';
