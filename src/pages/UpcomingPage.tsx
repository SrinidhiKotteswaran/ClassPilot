...
export function UpcomingPage() {
  ...
  const days = useMemo<DayGroup[]>(() => {
    ...
    for (let offset = -PAST_DAYS; offset <= FUTURE_DAYS; offset++) {
      ...
      groups.push({ date, label, isToday: offset === 0, items: [] });
    }
    ...
    for (const a of assignments) {
      ...
      if (diff >= -PAST_DAYS && diff <= FUTURE_DAYS) {
        const group = groups[diff + PAST_DAYS];
        group.items.push({ assignment: a, cls: a.class_id ? classById.get(a.class_id) : undefined });
      }
    }
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">
          {total} open assignments. Scroll through recent work and what is coming next.
        </p>
      </div>
